        ...
        if (
          ...
          strlen($confirm_password)>0
                ...
                if ($password===$confirm_password) {
                  ...
                  $sql = "SELECT users_id FROM users_login WHERE email=?";
                  ...
                  if ($n_row==0) {
                        // insert in the table
                        $sql = "INSERT INTO users_login (first_name,last_name,gender,email,user_type,phone,password) VALUES(?,?,?,?,?,?,?)";
                        $stmt = mysqli_prepare($connection, $sql);
                        mysqli_stmt_bind_param($stmt, 'sssssss', $first_name,$last_name,$gender,$email,$user_type,$phone,$password);
                        mysqli_stmt_execute($stmt);
